...
>
> **Version:** aligned with Collateral Agent spec v3.3. Output keys map to frontmatter fields and MDX body content in the Astro content collection (`src/content/case-studies/`).
...
Return a single JSON object with exactly these top-level keys, in this order, and nothing else. Each key maps to a frontmatter field in the Astro MDX content collection, except `story` which becomes the MDX body.
...
The `story` field is the MDX body, rendered inside the case-study layout's `article-prose` div (`src/layouts/CaseStudy.astro`). Only a subset of HTML is accepted; anything outside the allowlist is stripped or rendered incorrectly at build time.
