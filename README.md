...
> ⚠ **The daily cron is currently paused** (per Tyler, 2026-06-09) to stop
> steady-state Anthropic API spend while the project is on hold. See
> "Known gaps" below. Manual triggers — the operator portal, slash
> commands, and Issue Forms — are unaffected and still fully live.

## How it works

```
       ┌────────────────────────┐
       │   detect.yml           │  Normally runs on a daily schedule — currently
       │   (schedule: PAUSED)   │  paused, see "Known gaps". workflow_dispatch is
       └────────────┬───────────┘  still live, so it's runnable on demand from the
                    │              Actions tab. Queries PostHog (Fivetran-mirrored
                    │              postgres.campaigns) for funded slugs ≥ 2026-01-01,
...
.github/
  workflows/
    ...
    detect.yml            ← daily cron (currently paused; workflow_dispatch still live)
...
|---|---|---|
| [`.state/detection-log.md`](.state/detection-log.md) | Cron/detect.ts heartbeat (one row per run; PostHog-returned / already-published / eligible / generated / rate-limit deferred / failed) | First cron run after PR #29; appended on every `detect.ts` run since (scheduled or manually dispatched) — see "Known gaps" for the schedule's current paused status |
...
- **Daily cron is paused.** Per Tyler (2026-06-09), the `schedule:` triggers in `.github/workflows/detect.yml` are commented out to stop steady-state Anthropic API spend while the project is on hold — see the commit for the exact wording restored on resume. `workflow_dispatch` stays live, so `detect.ts` can still be run one-off from the Actions tab (with `dry-run` as an input) without editing the workflow file. `on-comment.yml` and `on-issue.yml` are untouched and still fire on any operator action (portal, slash commands, Issue Forms). Resume by uncommenting the `schedule:` block.
