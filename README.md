...
| **Cron heartbeat** (no-email file log; cron currently paused, see below) | [`.state/detection-log.md`](.state/detection-log.md) |

## How it works

> ⚠ **The daily cron is currently paused.** Per Tyler (2026-06-09), `detect.yml`'s
> `schedule` trigger is commented out to stop steady-state Anthropic API spend
> while the project is on hold — see "Known gaps" below. `workflow_dispatch`
> stays active, so detection can still be run one-off from the Actions tab, and
> all manual triggers (portal, slash commands, Issue Forms) are unaffected.
...
public/
  ...
  demo/                   ← noindex design demos (e.g. floating-cta.html), not linked from the live site
...
.github/
  workflows/
    ...
    detect.yml            ← cron, twice daily (currently paused — see "How it works")
...
- **Daily cron is paused.** Per Tyler (2026-06-09), the `schedule` triggers in `.github/workflows/detect.yml` are commented out to stop steady-state Anthropic API spend while the project is on hold. `workflow_dispatch` is untouched, so detection can still be run manually from the Actions tab (with or without `--dry-run`), and the portal / slash commands / Issue Forms are unaffected — only the automatic twice-daily scan is off. Resume by uncommenting the `schedule:` block in `detect.yml`.
