...
> ⚠ **The daily detection cron is currently paused.** Per operator request
> (Tyler, 2026-06-09), both scheduled slots in `detect.yml` are commented out
> to stop steady-state Anthropic spend while the project is on hold —
> see "Known gaps" below. `workflow_dispatch` still works, so detection and
> generation run fine on demand (portal / slash commands / Issue Forms /
> manual Actions dispatch); nothing publishes automatically until the cron
> is re-armed.
...
```
       ...
       └────────────┬───────────┘  Re-arm by uncommenting the schedule block;
...
scripts/
  ...
  lib/
    ...
    humanization-rules.ts ← single source of truth for banned phrases/openers;
                             feeds both humanize.ts and the runtime prompt injection
...
- **Daily cron is paused.** Both schedule slots in `.github/workflows/detect.yml` have been commented out since 2026-06-09 per operator request, to stop steady-state Anthropic spend while the project is on hold. `workflow_dispatch` remains enabled, so detection/generation still work on demand — the four case studies published since the pause (2026-06-09 and 2026-06-27) went through manual/backfill runs, not the cron. Resume by uncommenting the `schedule:` block in `detect.yml`.
