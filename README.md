...
> ⚠ **The daily cron is currently paused.** Per Tyler (2026-06-09), both
> `schedule` slots in `.github/workflows/detect.yml` are commented out to stop
> steady-state Anthropic spend while the project is on hold — see
> `.state/detection-log.md`, silent since 2026-06-01. `workflow_dispatch`
> stays live, so detection can still be run one-off from the Actions tab
> (or `npx tsx scripts/detect.ts`); resume the schedule by uncommenting the
> two `cron:` lines in `detect.yml`. Manual triggers (portal, slash commands,
> Issue Forms) are unaffected and still fully live.
...
.github/
  workflows/
    ...
    detect.yml            ← daily cron, currently paused (schedule commented out; see note above)
