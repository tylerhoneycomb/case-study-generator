...
```
       ...
       └────────────┬───────────┘  filters out already-published, newest first.
                    │              Runs at 04:47 UTC and 11:23 UTC (two slots guard
                    │              against GitHub Actions scheduler delays/drops).
                    ▼
       ┌────────────────────────┐  scripts/lib/pipeline.ts
       │   per-slug pipeline    │  scrape → Claude (prompts/case-study-prompt.md)
       │                        │  → humanization validator (retried once on fail;
       └────────────┬───────────┘    publishes with humanization-warning label if
                    │                still failing after 2 attempts) → image fetch
                    │              → MDX commit → push.
                    ▼
...
Running the agent CLIs locally needs `ANTHROPIC_API_KEY` and `GITHUB_TOKEN` in env. In CI both are wired up via repo secrets and `secrets.GITHUB_TOKEN`. Running `scripts/detect.ts` locally also requires `POSTHOG_API_KEY` and `POSTHOG_PROJECT_ID` (see `.env.example`).
...
scripts/
  ...
  lib/
    ...
    humanization-rules.ts ← banned phrases/openers/thresholds — single source of truth for validator + prompt
    humanize.ts           ← AI-tells regex validator built from humanization-rules.ts
...
.github/
  workflows/
    ...
    detect.yml            ← twice-daily cron (04:47 and 11:23 UTC)
...
|---|---|---|
| [`.state/detection-log.md`](.state/detection-log.md) | Per-run cron heartbeat (one row per run; PostHog-returned / already-published / eligible / generated / rate-limit deferred / failed). Two rows per UTC day when both cron slots fire. | First cron run after PostHog detection was wired up (PR #29); appended on every run thereafter |
...
- **Rate-limit persistence is per-run, not per-day.** `consume()` in `scripts/lib/ratelimit.ts` writes `.state/ratelimit.json` but the workflows don't commit it back to the repo, so each new workflow invocation starts with a fresh 0-of-3 budget. In practice this hasn't caused over-spend (backfill caps itself; manual ops are infrequent; the two daily cron slots are idempotent when both fire on a quiet day) but the documented "3/day across all triggers" semantic is more permissive than intended. Fix is small (add the file to per-slug commits in `pipeline.ts`); race conditions to think through if multiple workflows overlap.
