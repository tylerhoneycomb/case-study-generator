...
> ⚠ **Daily cron is paused.** Both `schedule` slots in `detect.yml` were commented out on 2026-06-09 per Tyler, to stop steady-state Anthropic API spend while the project is on hold — see `.github/workflows/detect.yml` for the note and how to resume. `workflow_dispatch` (manual run from the Actions tab) still works, and that's how case studies have been published since (backfill / manual generate). `.state/detection-log.md` last recorded an automated run on 2026-06-01; treat any gap after that as expected, not broken.
...
```
       ...
       │   (PAUSED, see above)  │  postgres.campaigns) for funded slugs ≥ 2026-01-01,
...
- **Daily cron is paused.** See the note under "How it works" above. Newly-funded campaigns are not being auto-detected or auto-published; publishing since 2026-06-09 has gone through manual `workflow_dispatch` / backfill only. Resume by uncommenting the `schedule` block in `.github/workflows/detect.yml`.
