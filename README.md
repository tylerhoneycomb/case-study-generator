...
```
       ...
       │   dispatch-only)       │  filters out already-published, newest first.
       └────────────┬───────────┘
                    │
                    ▼
...
> `schedule:` block is commented out; only `workflow_dispatch` remains active,
> so detection still runs on demand from the Actions tab but not on a timer.
> `on-comment.yml` and `on-issue.yml` are unaffected — manual ops (slash
> commands, Issue Forms, the portal) all still work normally. Resume by
...
scripts/
  ...
  lib/
    ...
    humanization-rules.ts ← single source of truth for banned phrases/openers;
                             both humanize.ts and claude.ts read from it
