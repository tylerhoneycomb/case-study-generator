...
> ⚠ **The daily cron is currently paused** (per Tyler, 2026-06-09) to stop steady-state
> Anthropic API spend while the project is on hold — see `.github/workflows/detect.yml`.
> `workflow_dispatch` still works, so it can be re-armed one-off from the Actions tab, or
> permanently by uncommenting the `schedule:` block in that file. Case studies published
> since the pause were all driven manually (portal / slash commands / Backfill). Last
> automated run: 2026-06-01 (see `.state/detection-log.md`).
...
- **Daily cron is paused, not removed.** See the callout above. This is an intentional
  operational state, not a bug — resume by re-enabling the `schedule:` block in
  `.github/workflows/detect.yml`. Anyone recreating this project from scratch should
  default the cron to **on** unless told otherwise; the pause reflects a point-in-time
  spend decision, not the intended steady state.
